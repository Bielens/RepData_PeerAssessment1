...
We will treat the `date` column as actual dates instead of strings. The `interval` column are not actual numbers, but a pointer into the 5-minute time interval. For convenience they will be treated a integer variables. 
...
To get an idea of the mean total number of steps taken per day, we first calculate the sum of the number of steps in all intervals for each day. For convenience these numbers will be stored in a data frame `totals`. The rows with missing values are ignored in this operation.
...
Note: numbers are rounded to the nearest whole number. Strangely enough the rounded values as calculated by the `mean()` and `median()` functions are `r as.integer(mean(totals$steps))` and `r as.integer(median(totals$steps))`, respectively, slightly different than the values in the `summary` overview! 
...
Highest mean activity (`r highestactivity` steps on average) is measured in the interval between
`r strftime(highestactivityinterval,"%H:%M")` and `r strftime(highestactivityinterval+5*60,"%H:%M")`.

Time series plot with an overview of the average daily activity:
```{r}
...
Each day in the set should have `r 24*12` rows containing data on a 5-minute interval in that day. The above result shows that if there is a day with missing values in the `steps` column, then **all** the values are missing for that day.

There are several possibl strategies to fill in the missing values.

1. Do nothing - since the missing values correspond to whole days, it could be argues that there the experiment did not include that day at all. E.g. the very first and last dates do not have any measurements, which could be interpreted as having a shorter experiment.
2. Replace the missing value by the mean or median for that given day. NOt an option here, because there are no values at all or the same day, let alone a median.
3. Replace the missing value by the mean or median for that specific interval across all dates in the experiment.

Option 1 does not give any extra information, and option 2 is not feasible, leaving only option 3 to investigate. The missing values will be replaced by the **median** of the values for the specific interval across all days. 
...
It turns out that the histogram of the total number of steps with the missing values filled in is almost exactly the same. The only difference is one spike near the 1000 steps area: the bin containing the counts for the 1000-1500 range now has count 8, where on the previous plot there were 0.
...
```{r}
# Assume english week names; if not, set locale to appropriate value..
...
Create a data frame with the intervals converted to time values, analogous to above:
```{r}
# Aggregate the data for use in the plot
means2 <- aggregate(steps ~ interval + daytype ,data=activity2, mean)
#For some reason coercion from factor to integer does not work as excpected, use explicit conversion
means3=data.frame(interval=means2$interval,
                  daytype=means2$daytype,
                  steps=means2$steps,
                  time=strptime(sprintf("%04d",as.integer(as.character(means2$interval))),"%H%M"))
```

Below is the plot for the average daily activity, split between weekdays and weekend days:

```{r}
# Create the line plot, use the daytype as facet
ggplot(means3,aes(x=time,y=steps))+
  facet_grid(daytype ~ .) +
